...
export default function AdminReclamacionesPage() {
  ...
  async function load() {
    setLoading(true);
    let query = supabase.from("reclamaciones").select("*").order("created_at", { ascending: false });
    if (filterEstado !== "todos") query = query.eq("estado", filterEstado);
    const { data, error } = await query;
    if (error) alert(error.message);
    else setRows((data as Reclamacion[]) || []);
    setLoading(false);
  }
  ...
  async function updateEstado(id: number, nuevoEstado: string) {
    ...
    if (error) alert(error.message);
      ...
      load();
      if (selected?.id === id) setSelected(prev => prev ? { ...prev, estado: nuevoEstado } : null);
    }
  }
    ...
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: "12px", overflow: "hidden" }}>
        {loading ? (
          ...
          <div style={{ padding: "48px", textAlign: "center", color: "#aaa", fontSize: "0.875rem" }}>No hay reclamaciones</div>
        ) : (
