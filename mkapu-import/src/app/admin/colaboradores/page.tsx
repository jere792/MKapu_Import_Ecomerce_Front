...
const initialForm = { name: "", logo_url: "", url: "", activo: true, orden: 0 };
...
export default function AdminColaboradoresPage() {
  ...
  async function save(e: React.FormEvent) {
    ...
    const payload = { name: form.name, logo_url: form.logo_url || null, url: form.url || null, activo: form.activo, orden: form.orden };
    const { error } = editId
  ...
  function onEdit(c: Colaborador) {
    setEditId(c.id);
    setForm({ name: c.name, logo_url: c.logo_url ?? "", url: c.url ?? "", activo: c.activo, orden: c.orden });
    setShowForm(true);
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "20px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
        <div>
          <h1 style={{ margin: 0, fontSize: "1.5rem", fontWeight: 700, color: "#1a1a1a" }}>Colaboradores</h1>
          <p style={{ margin: "4px 0 0", fontSize: "0.875rem", color: "#888" }}>{rows.length} colaborador{rows.length !== 1 ? "es" : ""} registrado{rows.length !== 1 ? "s" : ""}</p>
        </div>
      ...
      {showForm && (
        <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: "12px", padding: "24px", marginBottom: "28px", borderTop: "3px solid #f5a623" }}>
          <h2 style={{ margin: "0 0 20px", fontSize: "1rem", fontWeight: 700 }}>{editId ? "Editar colaborador" : "Nuevo colaborador"}</h2>
          <form onSubmit={save}>
            <div style={{ display: "grid", gridTemplateColumns: "2fr 2fr 1fr 1fr", gap: "16px", marginBottom: "16px" }}>
              <div>
                <label style={lbl}>Nombre *</label>
                <input className="fi" style={inp} placeholder="Nombre del colaborador" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
              </div>
              <div>
                <label style={lbl}>URL (enlace externo)</label>
                <input className="fi" style={inp} placeholder="https://..." value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} />
              </div>
              <div>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <button type="submit" className="bp" style={{ background: "#f5a623", color: "#fff", border: "none", padding: "10px 24px", borderRadius: "8px", fontWeight: 700, cursor: "pointer", fontSize: "0.875rem" }}>
                {editId ? "Guardar cambios" : "Crear colaborador"}
              </button>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem" }}>
            <thead>
              <tr style={{ background: "#fafafa", borderBottom: "1px solid #e8e8e8" }}>
                {["Logo", "Nombre", "URL", "Orden", "Estado", "Acciones"].map((h) => (
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={6} style={{ padding: "40px", textAlign: "center", color: "#aaa" }}>No hay colaboradores aún</td></tr>
              ) : rows.map((c, i) => (
                <tr key={c.id} className="rh" style={{ borderBottom: i < rows.length - 1 ? "1px solid #f0f0f0" : "none", background: "#fff" }}>
                  <td style={{ padding: "12px 16px" }}>
                    {c.logo_url ? <img src={c.logo_url} alt={c.name} style={{ height: 40, objectFit: "contain", borderRadius: "6px" }} /> : <span style={{ color: "#ccc", fontSize: "0.8rem" }}>Sin logo</span>}
                  </td>
                  <td style={{ padding: "12px 16px", fontWeight: 600, color: "#1a1a1a" }}>{c.name}</td>
                  <td style={{ padding: "12px 16px" }}>
                    {c.url ? <a href={c.url} target="_blank" rel="noopener noreferrer" style={{ color: "#f5a623", fontSize: "0.8rem", textDecoration: "none" }}>🔗 Visitar</a> : <span style={{ color: "#ccc", fontSize: "0.8rem" }}>—</span>}
                  </td>
                  <td style={{ padding: "12px 16px", color: "#888" }}>{c.orden}</td>
                  <td style={{ padding: "12px 16px" }}>
                    <span style={{ padding: "3px 10px", borderRadius: "20px", fontSize: "0.78rem", fontWeight: 700, background: c.activo ? "#e8f7ee" : "#fde8e8", color: c.activo ? "#1a7a3c" : "#a71d2a" }}>
                      {c.activo ? "Activo" : "Inactivo"}
                    </span>
                  </td>
                  <td style={{ padding: "12px 16px" }}>
                    <div style={{ display: "flex", gap: "6px" }}>
                      <button className="be" onClick={() => onEdit(c)} style={{ background: "rgba(0,123,255,0.08)", color: "#007bff", border: "1px solid rgba(0,123,255,0.2)", padding: "5px 12px", borderRadius: "6px", cursor: "pointer", fontSize: "0.8rem", fontWeight: 600 }}>Editar</button>
                      <button className="bd" onClick={() => onDelete(c.id)} style={{ background: "rgba(220,53,69,0.08)", color: "#dc3545", border: "1px solid rgba(220,53,69,0.2)", padding: "5px 12px", borderRadius: "6px", cursor: "pointer", fontSize: "0.8rem", fontWeight: 600 }}>Eliminar</button>
                    </div>
                  </td>
                </tr>
              ))}
