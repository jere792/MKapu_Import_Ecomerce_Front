...
export default function BlogForm({ mode, postId }: BlogFormProps) {
  ...
  const activeId = mode === "edit" ? postId ?? null : createdId;
  ...
  async function loadMedia(id: number) {
    ...
    setImagenes((imgs ?? []) as BlogImagen[]);
    setVideos((vids ?? []) as BlogVideo[]);
  }
  ...
  async function uploadFile(
    ...
    const folder = tipo === "imagen" ? "blog/imagenes" : "blog/videos";
  ...
  async function save(e: React.FormEvent, closeAfter = false) {
    ...
    if (activeId) {
      ...
      if (closeAfter) router.push("/admin/blog");
      return;
    }
      ...
      >
        ...
        {loading ? (
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: ".9rem" }}>Cargando post...</p>
          </div>
        ) : (
          <div className="ap-card ap-fadein">
            <form onSubmit={(e) => save(e)}>
              <div className="ap-fblock">
                <h3 className="ap-fblock__title">Información del post</h3>
                ...
                <div className="ap-fsub">
                  ...
                  >
                    ...
                    <div>
                      <label className="ap-lbl">Descripción corta</label>
                      <input
                        className="ap-inp"
                ...
                <div className="ap-fsub">
                  <h4 className="ap-fsub__title">Contenido</h4>
                  <textarea
                    className="ap-inp"
                ...
                <div className="ap-fsub">
                  ...
                  <label className="ap-toggle">
                    ...
                    <span>Activo (visible en el blog)</span>
                  </label>
                </div>
              </div>
            </form>
            ...
            if (activeId) {
              ...
              if (url) {
                await supabase.from("vlog_imagenes").insert({
                  vlog_post_id: activeId,
                  url_imagen: url,
                  orden: imgOrderRef.current++,
                });
              }
